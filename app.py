...
with col_input2:
    ...
    try:
        ...
        location = gps_location_button(buttonText="GPS 현재 위치로 검색")
        if location and location.get("latitude") and location.get("longitude"):
            lat = location["latitude"]
            lng = location["longitude"]
            if fire_addr:
                st.info(f"GPS 위치: {lat:.6f}, {lng:.6f}")
                if st.button("GPS 위치로 전환", key="gps_switch_btn"):
                    st.session_state["gps_clear_addr"] = True
                    st.query_params["gps_lat"] = str(lat)
                    st.query_params["gps_lng"] = str(lng)
                    st.rerun()
            elif fire_lat is None or fire_lng is None or abs(lat - fire_lat) > 0.000001 or abs(lng - fire_lng) > 0.000001:
                st.query_params["gps_lat"] = str(lat)
                st.query_params["gps_lng"] = str(lng)
                st.rerun()
            else:
    ...
    st.caption("버튼을 누르면 현재 위치가 자동 설정됩니다. (Chrome/Safari 필수)")
