...
GEO_HTML = """
<div style="width:100%;">
  <button id="geo-btn" style="width:100%;padding:14px;font-size:16px;font-weight:600;
    background-color:#ff4b4b;color:white;border:none;border-radius:8px;cursor:pointer;">
    GPS 현재 위치로 검색
  </button>
  <div id="geo-status" style="margin-top:8px;font-size:13px;color:#666;"></div>
</div>
<script>
...
btn.addEventListener('click', function() {
    if (!navigator.geolocation) {
        status.innerText = '이 브라우저는 위치 기능을 지원하지 않습니다.';
        return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
        function(pos) {
            ...
            try {
                ...
                topUrl.searchParams.set('gps_lat', lat);
                topUrl.searchParams.set('gps_lng', lng);
                window.top.location.href = topUrl.toString();
            } catch (e) {
                status.innerText = '위치는 확인했지만 페이지 이동 실패: ' + e;
                btn.disabled = false;
            }
        },
        function(err) {
            btn.disabled = false;
            if (err.code === 1) {
                status.innerText = '위치 권한 거부됨. 카카오톡 등 인앱 브라우저라면 우측 상단 메뉴에서 브라우저로 열기.';
            } else if (err.code === 2) {
                status.innerText = '위치 정보를 사용할 수 없습니다.';
            } else if (err.code === 3) {
                status.innerText = '위치 확인 시간 초과. 다시 시도해주세요.';
            } else {
                status.innerText = '위치를 가져올 수 없습니다.';
            }
        },
...
with col_input2:
    ...
    components.html(GEO_HTML, height=110)
    st.caption("버튼을 누르면 GPS 위치가 자동 설정됩니다. (Chrome/Safari 필수)")
