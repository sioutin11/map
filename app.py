...
if st.session_state.get("gps_clear_addr"):
    st.session_state["fire_addr_input"] = ""
    st.session_state["gps_clear_addr"] = False
...
with col_input1:
    ...
    fire_addr = st.text_input("화재 현장 주소를 입력하세요", placeholder="예: 충청북도 진천군 진천읍 교성리 1-1", key="fire_addr_input")
    if fire_addr:
        try:
...
with col_input2:
    ...
    if st.button("GPS 현재 위치로 검색", key="gps_search_btn"):
        st.session_state["gps_key"] += 1
        st.session_state["gps_clear_addr"] = True
        st.rerun()
    try:
        from streamlit_js_eval import streamlit_js_eval
        gps_key = st.session_state["gps_key"]
        if gps_key > 0 and not fire_addr:
            js_code = """
            new Promise((resolve) => {
                navigator.geolocation.getCurrentPosition(
                    pos => resolve(JSON.stringify({ok: true, lat: pos.coords.latitude, lng: pos.coords.longitude})),
                    err => resolve(JSON.stringify({ok: false, error: err.message})),
                    {enableHighAccuracy: true, timeout: 10000, maximumAge: 0}
                );
            })
            """
            result = streamlit_js_eval(js_expressions=js_code, key=f"gps_{gps_key}", want_output=True)
            if result:
                try:
                    import json
                    ...
                    if data.get("ok"):
                        lat = data["lat"]
                        lng = data["lng"]
                        if fire_lat is None or fire_lng is None or abs(lat - fire_lat) > 0.000001 or abs(lng - fire_lng) > 0.000001:
                            st.session_state["gps_key"] = 0
                            ...
                            st.session_state["gps_key"] = 0
                            st.success(f"GPS 위치 설정됨: {lat:.6f}, {lng:.6f}")
                    else:
                        st.warning(f"GPS 오류: {data.get('error', '알 수 없는 오류')}")
                except Exception:
                    st.info("GPS 위치를 가져오는 중...")
            else:
                st.info("GPS 위치를 가져오는 중...")
        elif fire_name and "GPS" in fire_name:
            st.success(f"GPS 위치 설정됨: {fire_lat:.6f}, {fire_lng:.6f}")
    except Exception as e:
        st.warning(f"GPS 모듈 로드 실패: {e}")
    st.caption("버튼을 누르면 현재 위치가 자동 설정됩니다.")
...
st.divider()
st.caption("소화전 탐색 시스템 | 충청북도 소화전 데이터")
